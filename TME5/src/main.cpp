...
void calcule(Color* pixels, int x, int y, vector<Vec3D> & lights, const Scene& scene, const Scene::screen_t & screen) {
...
class JobPixel : public Job {
	...
	public:
		...
		JobPixel(Color* pxl, int coordy, vector<Vec3D> & l, const Scene* sc, const Scene::screen_t* scr) {
			this->pixels = pxl;
			this->y = coordy;
			this->scene = sc;
			this->screen = scr;
		...
		void run(){
			...
			for (int x = 0; x < (*scene).getWidth(); ++x) {
				calcule(pixels, x, y, lights, *scene, *screen);
			}
...
int main () {
	...
	for (int  y = 0 ; y < scene.getHeight() ; y++) {
		p.submit(new JobPixel(pixels, y, lights, &scene, &screen));
	}

	p.stop();
	cout << p.nb_delete << endl;
